...
$cert = New-SelfSignedCertificate `
    ...
    -FriendlyName "UnionDuesManager Signing" `
...
Export-Certificate -Cert $cert -FilePath "$desktop\UnionDuesManager_signing.cer"
Write-Host "CER を出力しました: $desktop\UnionDuesManager_signing.cer"
```

> **注意:** `signing.pfx` は GitHub Secrets 登録後に削除してください（漏洩防止）。
> `UnionDuesManager_signing.cer` は各 PC への初回インストール時に必要なので保管しておきます。
...
`UnionDuesManager_signing.cer` を渡してインストールしてもらいます：
...
| `UnionDuesManager_signing.cer` | 開発者が保管・配布 | エンドユーザーへの証明書配布用 |
